...
/**
 * Create UC (controlled airspace) layout parser.
 * @param {object} config
 * @param {string} [layoutName]
 * @returns {{parse: Function}}
 */
export function createUcLayout(config, layoutName = "UC") {
  const s = makeLayoutSlicer(config, layoutName);
  return {
    /**
     * Parse a UC line into a record.
     * @param {string} line
     * @returns {object|null}
     */
    parse(line) {
      ...
      const airspaceType = s.trim(line, "airspaceType");
      const airspaceCenter = s.trim(line, "airspaceCenter");
      const classification = s.trim(line, "classification");
